...
export function SupplyChainTimeline({ 
  ...
  const checkpoints: CheckpointData[] = [
    ...
    ...distributors.map((dist, idx) => ({
      ...
      address: dist,
      timestamp: createdAt + (idx + 1) * 86400,
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'distributor': return <Truck className="w-6 h-6" />;
  ...
  const getTitle = (type: string) => {
    switch (type) {
      case 'manufacturer': return 'Manufactured';
      case 'distributor': return 'Distributed';
  ...
  return (
    <div className="card">
      ...
      <div className="relative pl-12">
        {/* Timeline Line */}
        <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-primary-500 via-blue-500 to-success-500"></div>
            ...
            <div key={index} className="relative flex items-start space-x-6">
              ...
              <div className="relative flex-shrink-0 z-10">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg 
                  ${checkpoint.verified ? 'bg-gradient-to-br from-primary-500 to-blue-500' : 'bg-slate-700'}`}>
                  <div className="text-white">{getIcon(checkpoint.type)}</div>
                </div>
                {checkpoint.verified && (
              ...
              <div className="flex-1 bg-slate-800/50 rounded-lg p-4 border border-slate-700/50 hover:border-primary-500/50 transition-colors">
                ...
                {checkpoint.verified && (
                  <div className="mt-3 pt-3 border-t border-slate-700/50 flex flex-col md:flex-row md:justify-between md:items-center gap-2 text-xs">
                    ...
                    <span className="text-slate-500 mt-1 md:mt-0">AI Confidence: 98%</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {checkpoints.length < 5 && (
            <div className="relative flex items-start space-x-6 opacity-40">
              ...
              <div className="flex-1 bg-slate-800/30 rounded-lg p-4 border border-dashed border-slate-700/50">
                ...
                <p className="text-sm text-slate-600 mt-1">Final destination not yet recorded</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mt-6 pt-6 border-t border-slate-700">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          ...
          <div>
            <p className="text-2xl font-bold text-success-400">{checkpoints.filter(c => c.verified).length}</p>
            <p className="text-sm text-slate-400">Verified</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-blue-400">{Math.floor((Date.now() / 1000 - createdAt) / 86400)}</p>
